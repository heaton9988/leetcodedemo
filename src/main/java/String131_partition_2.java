...
public class String131_partition_2 {
    ...
    private void backtracking(String s, int start, int len, Deque<String> path, List<List<String>> res) {
        ...
        for (int end = start; end < len; end++) {
            ...
            // 不是的话，剪枝
            System.out.println("start=" + start + "\tend=" + end + "\t" + checkPalindrome(s, start, end));

            if (checkPalindrome(s, start, end)) {
                path.addLast(s.substring(start, end + 1));
                backtracking(s, end + 1, len, path, res);
                path.removeLast();
            }
        }
    }
    ...
    public static void main(String[] args) {
        Object o = new String131_partition_2().partition("aab");
